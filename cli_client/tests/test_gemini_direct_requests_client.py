...
class TestChatWithModel:
    ...
    def test_chat_with_model_success(self, mock_post):
        ...
        # テスト実行
        result = gemini_client.chat_with_model("こんにちは", "gemini-2.0-flash")
    ...
    def test_chat_with_model_error(self, mock_post):
        ...
        # テスト実行
        result = gemini_client.chat_with_model("エラーを発生させるプロンプト")
...
class TestAnalyzeImage:
    @patch('gemini_client.requests.post')
    @patch('gemini_client.get_base64_encoded_image')
    @patch('gemini_client.Image.open')
    @patch('gemini_client.os.path.splitext')
    def test_analyze_image_success(self, mock_splitext, mock_image_open, mock_get_base64, mock_post):
        ...
        # テスト実行
        result = gemini_client.analyze_image("test_image.jpg", "この画像は何ですか？")
    ...
    def test_analyze_image_error(self, mock_open):
        ...
        with pytest.raises(FileNotFoundError):
            gemini_client.analyze_image("non_existent_image.jpg")


class TestGenerateImage:
    @patch('gemini_client.requests.post')
    @patch('gemini_client.os.makedirs')
    ...
    def test_generate_image_success(self, mock_b64decode, mock_file, mock_makedirs, mock_post):
        ...
        # テスト実行
        result = gemini_client.generate_image("綺麗な富士山", output_path)
    ...
    def test_generate_image_error(self, mock_post):
        ...
        # テスト実行
        result = gemini_client.generate_image("エラーを発生させるプロンプト")
...
class TestGetBase64EncodedImage:
    ...
    def test_get_base64_encoded_image(self, mock_file):
        ...
        # テスト実行
        result = gemini_client.get_base64_encoded_image("test_image.jpg")
...
class TestMain:
    @patch('gemini_client.chat_with_model')
    @patch('gemini_client.analyze_image')
    @patch('gemini_client.generate_image')
    @patch('gemini_client.argparse.ArgumentParser.parse_args')
